...
@section('title', 'Edit Konten Gallery - SIMBRO Admin')
    ...
    <div class="bg-gradient-to-br from-[#FF7A1D] to-[#CD5500] text-white px-6 py-6 md:px-10">
        <div class="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <div class="flex items-center gap-4">
                ...
                <div>
                    <h1 class="text-2xl font-bold">Edit Konten Gallery</h1>
                    <p class="text-orange-100 text-sm">Perbarui informasi konten gallery</p>
                </div>
            </div>
            <div>
                <i class="fas fa-arrow-left"></i>
                <a href="{{ route('admin.home', ['#gallery']) }}" class="inline-flex items-center gap-2 text-white hover:underline text-sm font-bold">Kembali ke Gallery</a>
            </div>
        </div>
    </div>

    <div class="max-w-4xl mx-auto py-8 px-4">
        <form method="POST" action="{{ route('admin.gallery.update', $gallery->gallery_id) }}" enctype="multipart/form-data" id="formGallery" novalidate>
            @csrf @method('PUT')
            <div class="bg-white rounded-xl shadow-md border border-gray-200 p-6 space-y-6">
                <h2 class="text-xl font-bold text-gray-800 mb-5 border-l-4 border-[#FF6B00] pl-3">Buat Perubahan Konten Gallery</h2>
                ...
                <div>
                    ...
                    <input type="text" name="judul" id="judul" value="{{ old('judul', $gallery->judul) }}" class="w-full rounded-lg border border-gray-300 px-3 py-2 focus:ring-2 focus:ring-orange-500 focus:border-orange-500">
                ...
                <div>
                    ...
                    <input type="text" name="keterangan" id="keterangan" value="{{ old('keterangan', $gallery->keterangan) }}" class="w-full rounded-lg border border-gray-300 px-3 py-2 focus:ring-2 focus:ring-orange-500 focus:border-orange-500">
                    <div class="flex justify-between items-center mt-1">
                ...
                <div>
                    <label class="block font-semibold text-gray-800 mb-1">Artikel <span class="text-sm font-normal text-gray-500">Maksimal 2000 karakter, termasuk spasi</span></label>
                    <textarea name="artikel" id="editor" rows="10" class="w-full rounded-lg border border-gray-300 px-3 py-2 focus:ring-2 focus:ring-orange-500 focus:border-orange-500">{{ old('artikel', $gallery->artikel) }}</textarea>
                    <div class="flex justify-between items-center mt-1">
...
<script>
    ...
    ClassicEditor.create(document.querySelector('#editor'), {
        ...
        initialData: @json(old('artikel', $gallery->artikel))
    }).then(editor => {
    ...
    form.addEventListener('submit', function(e) {
        ...
        if (!validation) {
            ...
            // Sync CKEditor ke textarea sebelum FormData dibuat
            if (editorInstance) {
                ...
                if (data.success) {
                    ...
                    if (data.errors) {
                        ...
                        showLightboxCustom(message, 'error', () => {
                            clearErrors();
                            // Tampilkan error per field dari server
                            for (let key in data.errors) {
